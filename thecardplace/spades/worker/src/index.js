/* Spades - the Cloudflare edge of the room.
 *
...
export class SpadesRoom {
  ...
  ensureRoom(config) {
    ...
    this.room = SH.Room.create({
       ...
       *
       * Two different questions, and answering them with one number is what
       * takes a present player's cards off them. Ninety seconds is an ordinary
       * length of time to spend deciding whether to order it up when the hand
       * has to be read aloud first, and a client that is still answering its
       * pings has not stopped responding, whatever it is doing with its turn.
       *
...
/* What a room plays by when nothing said otherwise.
 *
 * Reached on two paths: a room created before this field existed, and a wake
 * where the stored config could not be read. Neither should produce a table
 * nobody can play, so these are the ordinary rules of spades rather than
 * anything clever. Four seats is not a default, it is the game. */
function defaultConfig() {
  /* Spades is always four seats, and the partnerships are decided by where
   * people sit rather than chosen — seats 0 and 2 against 1 and 3 — so there is
   * no table size and no pairing to carry here.
   *
   * Every rule the engine reads IS here, and that matters more than it looks. A
   * room waking on this path with only pointsToWin set would score bags and
   * nils by the engine's own fallbacks. They are the same numbers today, which
   * is exactly what makes the omission invisible until somebody changes one of
   * them in game.js and this silently disagrees. */
  return {
    numPlayers: 4,
